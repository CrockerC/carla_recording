...
buf = []
fid = 0
scale = 2.5  # how much more resolution you want to have than the final product, this is needed because carla's image quality is bad.
             # so running at a higher resolution and then downscaling it gives you a much better image than running it at your final resolution natively
camera_res = (str(640 * scale), str(480 * scale))  # resolution of the dash cam
time_step = 1 / 20  # time step per frame


def fast_save_to_disk(path, image, velocity):
    # simplist thing to do here was use two globals, kept from having to create a new class and deal with it in the code where the images are captured
    global buf
    global fid
    ...
    # if the buffer has 20 frames in it, flush it to the disk with a thread and increment the fid
    if len(buf) >= 20:
        threading.Thread(target=save_data, args=(buf.copy(), 'rawFrames\\' + str(fid))).start()
        buf = []
        fid += 1
...
def save_data(raw_data, path):
    ...
    # 0: path
    # 1: frame data
    # 2: size of the frame (L x W)
    # 3: speed of the car at the frame
    data = []
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        # MY CODE
        # set each frame to be a fixed time step of .05 seconds, aka 20 fps
    ...
    def destroy(self):
        sensors = self.camera_manager.sensors
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        # MY CODE
        self.frametime = time_step  # im pretty sure that this is broken in carla, but im afraid to remove it
        ...
        for i, item in enumerate(self.sensorsRAW):
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            # if the sensor is the one on the dash, record
            sensor.listen(lambda image: CameraManager._parse_image(weak_self, image, index == 1))
    ...
    @staticmethod
    def _parse_image(weak_self, image, record):
        self = weak_self()
        if not self:
            return
        ...
        if self.recording and record:
            ...
            # check for skipped frames, for video quality purposes
            delta_t = image.timestamp - self.lastTime
            self.total_frames += 1
            if delta_t > time_step + .01:
